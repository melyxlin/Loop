                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            HStack(spacing: 4) {
                                Text(formatEventTime(match.eventStart))
                                    .font(.caption)
                                if let preset = coordinator.availablePresets().first(where: { $0.id == match.trigger.presetId }) {
    ...
    private func triggerRow(_ trigger: AutoPresetsCalendarTrigger) -> some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                if let preset = coordinator.availablePresets().first(where: { $0.id == trigger.presetId }) {
                    Text("→ \(preset.symbol) \(preset.name)")
                        .font(.caption)
                        .foregroundColor(.secondary)
                } else {
...
struct AutoPresets_CalendarTriggerEditView: View {
    ...
    private let availablePresets: [TemporaryPreset]
    private var isEditing: Bool { existingTrigger != nil }

    init(trigger: AutoPresetsCalendarTrigger?, availablePresets: [TemporaryPreset]) {
