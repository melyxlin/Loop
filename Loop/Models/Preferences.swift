...
struct Preferences: PreferencesProvider {
    ...
    var basalLockThreshold: LoopQuantity {
        get {
            let key = "basalLockThreshold"
            if let value = UserDefaults.standard.value(forKey: key) as? Double {
                return LoopQuantity(unit: .milligramsPerDeciliter, doubleValue: value)
            } else {
                return LoopQuantity(unit: .milligramsPerDeciliter, doubleValue: 250)
            }
        }
        set {
            let key = "basalLockThreshold"
