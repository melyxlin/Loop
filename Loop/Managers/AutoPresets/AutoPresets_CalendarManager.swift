...
public final class AutoPresets_CalendarManager: NSObject, ObservableObject {
    ...
    private var activatedByCalendar: [String: String] = [:]
    ...
    private func activatePreset(for trigger: AutoPresetsCalendarTrigger, event: EKEvent) {
        guard !trigger.presetId.isEmpty else { return }

        let presetId = trigger.presetId
        let coordinator = AutoPresets_Coordinator.shared

        // Don't override a manually-set preset
        if let _ = coordinator.currentOverride(),
        ...
        guard let preset = coordinator.availablePresets().first(where: { $0.id == presetId }) else {
            os_log("Preset ID %{public}@ not found", log: log, type: .error, trigger.presetId)
            return
        }

        activatedByCalendar[trigger.id.uuidString] = presetId
        coordinator.delegate?.autoPresets(coordinator, shouldActivatePreset: preset)

        let storage = AutoPresets_Storage()
        storage.addLogEntry(
            event: .presetActivated,
            activityType: nil,
        ...
        os_log(
            "Calendar activated preset '%{public}@' for event '%{public}@'",
            log: log,
            type: .info,
            preset.name,
            event.title ?? ""
    ...
    private func deactivatePreset(for trigger: AutoPresetsCalendarTrigger, event: EKEvent) {
        guard let presetId = activatedByCalendar[trigger.id.uuidString] else { return }
        ...
        os_log(
            "Calendar deactivated preset '%{public}@' after event '%{public}@' ended",
            log: log,
            type: .info,
            preset.name,
            event.title ?? ""
