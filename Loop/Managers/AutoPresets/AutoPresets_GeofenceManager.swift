...
public final class AutoPresets_GeofenceManager: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    private var activatedByGeofence: [String: String] = [:]
    ...
    private func activatePreset(for location: AutoPresetsGeofenceLocation) {
        ...
        guard let preset = coordinator.availablePresets().first(where: { $0.id == presetId }) else {
            os_log("Preset UUID %{public}@ not found in available presets", log: log, type: .error, location.presetId)
            return
        }

        activatedByGeofence[location.id.uuidString] = presetId
    ...
    private func deactivatePreset(for location: AutoPresetsGeofenceLocation) {
        guard let presetId = activatedByGeofence[location.id.uuidString] else {
...
extension Notification.Name {
    static let autoPresetsGeofenceLocationUpdated =
        Notification.Name("com.loopkit.Loop.autoPresetsGeofenceLocationUpdated")

    static let autoPresetsPresetActivated =
        Notification.Name("com.loopkit.Loop.autoPresetsPresetActivated")

    static let autoPresetsPresetDeactivated =
        Notification.Name("com.loopkit.Loop.autoPresetsPresetDeactivated")
}
